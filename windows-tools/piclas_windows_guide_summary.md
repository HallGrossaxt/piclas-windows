...
Port major version tracks the upstream PICLas minor era: **1.x** = PICLas 4.1.0, **2.x** = PICLas 4.2.0.
...
| **v1.5** | June 2026 | Fixed the broken DSMC+GPU release build from v1.4 ("Could not find nvcc"): push/tag builds skipped the CUDA install while selecting the GPU preset (→ `cpu_only` truthiness input), and the space-containing nvcc path was word-split (→ bash array). Docs now recommend the no-GPU build for most (GPU only speeds the particle push). |
| **v1.4** | June 2026 | Self-contained release zips — bundle `msmpi.dll` (MPI build runs without a separate MS-MPI install); replaced the no-MPI variant with a CPU-only (no-GPU) fallback. |
| **v1.3** | June 2026 | Release repackaging: two consolidated zips (MPI/no-MPI) with per-tool folders (`piclas-win/`, `piclas2vtk/`, `superB/`) + ready-to-run `example/`. Fat GPU binaries (sm_75/86/89 + compute_90 PTX) — native Turing–Ada (fixes RTX 4500 Ada CPU-fallback), JIT beyond. |
| **v1.2** | June 2026 | Ship `dsmc-cone-2D` as a ready-to-run release example (self-contained: `parameter.ini` + `DSMC.ini` + pre-converted CGNS 5-BC mesh + README; no HOPR needed), covered by `SHA256SUMS.txt`. CGNS = the 5-BC mesh the stock ini expects (Gmsh's 6-BC is incompatible). |
| **v1.1** | June 2026 | First portable Windows release — packaged DSMC+MPI+GPU `piclas-win.exe` + `piclas2vtk` + `superB` as a downloadable release; CI forces LF on `*.sh` for Windows runners. |
| **v1.0** | May 2026 | Major release: Bug G load-balance out-of-bounds fix; executable renamed `piclas.exe` → `piclas-win.exe` (GPLv3 §5a unofficial-port marking). Cumulative §16 MS-MPI / serial-HDF5 fixes. |
