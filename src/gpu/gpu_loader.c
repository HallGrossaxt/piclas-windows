...
void piclas_gpu_load_library(void)
{
    ...
    g_gpu_init      = (pfn_init)     resolve("piclas_gpu_init");
    ...
    g_gpu_query_max = (pfn_query_max)resolve("piclas_gpu_query_max_safe");
...
{
    g_gpu_init(localRank, localSize);
}
                             ...
                             double RK_a, double b_dt)
{
    g_gpu_lserk(PartState, Pt_temp, Pt, isActive, isPush, isNewPart,
                nPart, isStage1, isLastStage, ptTempResident, RK_a, b_dt);
}
