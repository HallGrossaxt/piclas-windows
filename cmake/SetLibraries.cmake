...
# MPI
# =========================================================================
# On Windows, help CMake find Microsoft MPI (MS-MPI) if present
IF(WIN32)
  # MS-MPI installs to MSMPI_ROOT or the default Program Files location
  IF(DEFINED ENV{MSMPI_ROOT})
    SET(MPI_HOME "$ENV{MSMPI_ROOT}" CACHE PATH "MS-MPI installation root")
  ELSEIF(EXISTS "C:/Program Files/Microsoft MPI")
    SET(MPI_HOME "C:/Program Files/Microsoft MPI" CACHE PATH "MS-MPI installation root")
  ENDIF()
  # MS-MPI SDK for headers/import libs
  IF(DEFINED ENV{MSMPI_INC})
    LIST(APPEND CMAKE_PREFIX_PATH "$ENV{MSMPI_INC}/..")
  ENDIF()
ENDIF()
...
IF(LIBS_USE_MPI)
  ...
  ELSEIF(MPI_C_LIBRARY_VERSION_STRING MATCHES ".*Intel.*" AND MPI_C_VERSION_MAJOR VERSION_EQUAL "3")
    ...
    STRING(REGEX MATCH "([0-9]+)\\.([0-9]+)" MPI_C_LIBRARY_VERSION ${MPI_C_LIBRARY_VERSION_STRING})
  ELSEIF(WIN32 AND MPI_C_VERSION_MAJOR VERSION_GREATER_EQUAL "2")
    # Microsoft MPI (MS-MPI) on Windows - version string format differs
    SET(LIBS_MPI_NAME "MS-MPI")
    IF(MPI_C_LIBRARY_VERSION_STRING MATCHES "([0-9]+)\\.([0-9]+)")
      SET(MPI_C_LIBRARY_VERSION "${CMAKE_MATCH_1}.${CMAKE_MATCH_2}")
    ELSE()
      SET(MPI_C_LIBRARY_VERSION "${MPI_C_VERSION_MAJOR}.${MPI_C_VERSION_MINOR}")
    ENDIF()
    # MS-MPI uses MPICH under the hood; apply the same argument-mismatch fix for GNU
    IF (CMAKE_Fortran_COMPILER_ID MATCHES "GNU")
      SET(CMAKE_Fortran_FLAGS "${CMAKE_Fortran_FLAGS} -fallow-argument-mismatch")
    ENDIF()
  ELSE()
    MESSAGE(FATAL_ERROR "Cannot detect supported MPI type or version. Valid options are Cray MPICH, IntelMPI, MPICH, MS-MPI (Windows), HPE MPT, and OpenMPI supporting MPI version 3.x")
  ENDIF()
...
# =========================================================================
# MS-MPI compatibility: mpi_f08.mod (Windows only)
# =========================================================================
# MS-MPI does not ship a Fortran 2008 interface (no mpi_f08.mod).
# We bundle cmake/mpi_f08.f90 which provides typed handle wrappers on top
# of MS-MPI's legacy MODULE MPI.  Compile it once at configure time.
IF(WIN32 AND LIBS_USE_MPI)
  SET(_mpi_f08_dir "${CMAKE_BINARY_DIR}/mpi_f08_compat")
  SET(_mpi_f08_src "${CMAKE_CURRENT_SOURCE_DIR}/cmake/mpi_f08.f90")

  # Check if mpi_f08.mod already exists somewhere on the MPI include path
  SET(_mpi_f08_mod_found FALSE)
  FOREACH(_inc IN LISTS MPI_Fortran_INCLUDE_PATH MPI_INCLUDE_PATH)
    IF(EXISTS "${_inc}/mpi_f08.mod")
      SET(_mpi_f08_mod_found TRUE)
      MESSAGE(STATUS "[MPI] Found system mpi_f08.mod in ${_inc} – skipping bundled wrapper")
    ENDIF()
  ENDFOREACH()
  ...
  IF(NOT _mpi_f08_mod_found AND NOT EXISTS "${_mpi_f08_dir}/mpi_f08.mod")
    ...
    ENDIF()
    MESSAGE(STATUS "[MPI] Compiling bundled mpi_f08.f90 compatibility wrapper")
    FILE(MAKE_DIRECTORY "${_mpi_f08_dir}")
    SET(_mpi_inc_flags "")
    FOREACH(_inc IN LISTS MPI_Fortran_INCLUDE_PATH MPI_INCLUDE_PATH)
      LIST(APPEND _mpi_inc_flags "-I${_inc}")
    ENDFOREACH()
    EXECUTE_PROCESS(
    ...
    IF(NOT _result EQUAL 0)
      MESSAGE(FATAL_ERROR "[MPI] Failed to compile bundled mpi_f08.f90:\n${_err}")
    ENDIF()
    MESSAGE(STATUS "[MPI] mpi_f08.mod generated successfully")
  ENDIF()

  # Put compat dir first so 'USE mpi_f08' resolves before any system path
  IF(EXISTS "${_mpi_f08_dir}/mpi_f08.mod" OR _mpi_f08_mod_found)
    IF(NOT _mpi_f08_mod_found)
      INCLUDE_DIRECTORIES(BEFORE "${_mpi_f08_dir}")
      # Link the compiled wrapper object so subroutine/function bodies are available
      # INSERT at position 0 so mpi_f08.o precedes libmsmpi.dll.a in the link command;
      # GNU ld processes archives left-to-right so the object's undefined refs (mpi_wtick_ etc.)
      # must appear before the import library that resolves them.
...
IF(NOT LIBS_BUILD_HDF5)
  ...
  IF("${HDF5_Fortran_LIBRARY_hdf5_fortran}" STREQUAL "")
    ...
    IF(WIN32)
      CMAKE_PATH(APPEND HDF5_ROOT_DIR "lib/libhdf5_fortran.dll.a" OUTPUT_VARIABLE HDF5_Fortran_LIBRARY_hdf5_fortran)
      IF(NOT EXISTS "${HDF5_Fortran_LIBRARY_hdf5_fortran}")
        CMAKE_PATH(SET HDF5_Fortran_LIBRARY_hdf5_fortran "${HDF5_ROOT_DIR}/lib/libhdf5_fortran.a")
      ENDIF()
    ELSEIF(APPLE)
      CMAKE_PATH(APPEND HDF5_ROOT_DIR "lib/libhdf5_fortran.dylib" OUTPUT_VARIABLE HDF5_Fortran_LIBRARY_hdf5_fortran)
    ELSE()
  ...
  IF(LIBS_USE_MPI)
    # Use nm on macOS and Windows (readelf is Linux/ELF only)
    # Search specifically for h5pset_fapl_mpio_f to avoid false positives from h5fd_mpio_f
    IF(APPLE)
      EXECUTE_PROCESS(COMMAND nm -gU ${HDF5_Fortran_LIBRARY_hdf5_fortran} COMMAND grep h5pset_fapl_mpio_f   OUTPUT_VARIABLE HDF5_USES_MPIF   RESULT_VARIABLE GREP_RESULT OUTPUT_STRIP_TRAILING_WHITESPACE)
    ELSEIF(WIN32)
      EXECUTE_PROCESS(COMMAND nm -g  ${HDF5_Fortran_LIBRARY_hdf5_fortran} COMMAND grep h5pset_fapl_mpio_f   OUTPUT_VARIABLE HDF5_USES_MPIF   RESULT_VARIABLE GREP_RESULT OUTPUT_STRIP_TRAILING_WHITESPACE)
    ELSE()
      EXECUTE_PROCESS(COMMAND readelf -Ws ${HDF5_Fortran_LIBRARY_hdf5_fortran} COMMAND grep h5pset_fapl_mpio_f   OUTPUT_VARIABLE HDF5_USES_MPIF   RESULT_VARIABLE GREP_RESULT OUTPUT_STRIP_TRAILING_WHITESPACE)
    ENDIF()
    ...
    ELSEIF(WIN32)
      EXECUTE_PROCESS(COMMAND nm -g  ${HDF5_Fortran_LIBRARY_hdf5_fortran} COMMAND grep mpio_f08 OUTPUT_VARIABLE HDF5_USES_MPIF08 RESULT_VARIABLE GREP_RESULT OUTPUT_STRIP_TRAILING_WHITESPACE)
    ELSE()
...
ELSE()
  ...
  # Determine shared library extension and existence check filename for the platform
  IF(WIN32)
    SET(HDF5_SHARED_SUFFIX ".dll.a")
    SET(HDF5_SHARED_CHECK  "bin/libhdf5-0.dll")
  ELSEIF(APPLE)
    SET(HDF5_SHARED_SUFFIX ".dylib")
    SET(HDF5_SHARED_CHECK  "lib/libhdf5.dylib")
  ELSE()
    SET(HDF5_SHARED_SUFFIX ".so")
    SET(HDF5_SHARED_CHECK  "lib/libhdf5.so")
  ENDIF()
    ...
    IF(WIN32)
      ...
      EXTERNALPROJECT_ADD(HDF5
        ...
        CMAKE_ARGS
          ...
          -DHDF5_ENABLE_HL_LIB=ON
          -DBUILD_SHARED_LIBS=ON
          -DHDF5_BUILD_STATIC_LIBS=ON
          ${LIBS_HDF5_MPI_ARGS}
        BUILD_BYPRODUCTS
          ${LIBS_HDF5_DIR}/lib/libhdf5_fortran.a
          ${LIBS_HDF5_DIR}/lib/libhdf5.a
          ${LIBS_HDF5_DIR}/lib/libhdf5_fortran${HDF5_SHARED_SUFFIX}
          ${LIBS_HDF5_DIR}/lib/libhdf5${HDF5_SHARED_SUFFIX}
          ${LIBS_HDF5_DIR}/bin/h5diff
      )
      SET(LIBS_HDF5_CMAKE TRUE)
    ELSE()
      # Optional Features:
  ...
  ENDIF()
  # Always set for self-build libraries (configure-based), CMAKE-based sets LIBS_HDF5_CMAKE=TRUE above
  IF(NOT WIN32)
    SET(LIBS_HDF5_CMAKE FALSE)
  ENDIF()

  # Set HDF5 paths (platform-aware shared library extensions)
  ...
  SET(HDF5_hdf5_LIBRARY_hdf5                    ${LIBS_HDF5_DIR}/lib/libhdf5${HDF5_SHARED_SUFFIX})
  SET(HDF5_hdf5_LIBRARY_RELEASE                 ${LIBS_HDF5_DIR}/lib/libhdf5.a)
  SET(HDF5_Fortran_LIBRARY_hdf5_fortran         ${LIBS_HDF5_DIR}/lib/libhdf5_fortran${HDF5_SHARED_SUFFIX})
...
# USE_MPI_HDF5 guards calls to H5PSET_FAPL_MPIO_F and H5PSET_DXPL_MPIO_F which require parallel HDF5
...
ELSE()
  ...
  # Platform-specific shared library extension and filenames
  IF(WIN32)
    SET(MATHLIB_SHARED_EXT ".dll")
    SET(MATHLIB_IMPORT_EXT ".dll.a")
  ELSEIF(APPLE)
    SET(MATHLIB_SHARED_EXT ".dylib")
    SET(MATHLIB_IMPORT_EXT ".dylib")
  ELSE()
    SET(MATHLIB_SHARED_EXT ".so")
    SET(MATHLIB_IMPORT_EXT ".so")
  ENDIF()

  IF (LIBS_BUILD_MATH_LIB_VENDOR MATCHES "LAPACK")
    # Check if math lib was already built
    IF (NOT EXISTS "${LIBS_MATH_DIR}/lib/liblapack${MATHLIB_IMPORT_EXT}")
      # Let CMake take care of download, configure and build
      EXTERNALPROJECT_ADD(${LIBS_BUILD_MATH_LIB_VENDOR}
        ...
        BUILD_BYPRODUCTS ${LIBS_MATH_DIR}/lib/liblapack${MATHLIB_IMPORT_EXT} ${LIBS_MATH_DIR}/lib/libblas${MATHLIB_IMPORT_EXT}
      )
  ...
  ELSEIF (LIBS_BUILD_MATH_LIB_VENDOR MATCHES "OpenBLAS")
    # Check if math lib was already built
    IF (NOT EXISTS "${LIBS_MATH_DIR}/libopenblas${MATHLIB_IMPORT_EXT}")
      # Let CMake take care of download, configure and build
      EXTERNALPROJECT_ADD(${LIBS_BUILD_MATH_LIB_VENDOR}
        GIT_REPOSITORY ${MATH_LIB_DOWNLOAD}
        GIT_TAG ${MATH_LIB_TAG}
        ...
        UPDATE_COMMAND ""
        CONFIGURE_COMMAND ""
        BUILD_BYPRODUCTS ${LIBS_MATH_DIR}/src/${LIBS_BUILD_MATH_LIB_VENDOR}/libopenblas${MATHLIB_IMPORT_EXT}
  ...
  IF (LIBS_BUILD_MATH_LIB_VENDOR MATCHES "LAPACK")
    ...
    SET(LAPACK_LIBRARY                  ${MATH_LIB_LIBRARIES}/liblapack${MATHLIB_IMPORT_EXT})
    SET(BLAS_LIBRARY                    ${MATH_LIB_LIBRARIES}/libblas${MATHLIB_IMPORT_EXT})
  ...
  ELSEIF (LIBS_BUILD_MATH_LIB_VENDOR MATCHES "OpenBLAS")
    ...
    SET(LAPACK_LIBRARY                  ${MATH_LIB_LIBRARIES}/libopenblas${MATHLIB_IMPORT_EXT})
    SET(LAPACK_LIBRARIES                ${LAPACK_LIBRARY}${BLAS_LIBRARY})
...
IF(LIBS_DOWNLOAD_HOPR)
  ...
  IF(NOT EXISTS "${CMAKE_CURRENT_BINARY_DIR}/bin/hopr")
    ...
    ELSE()
      IF(WIN32)
        EXECUTE_PROCESS(COMMAND ${CMAKE_COMMAND} -E copy ${HOPR_APP_FILE} bin/hopr)
      ELSE()
