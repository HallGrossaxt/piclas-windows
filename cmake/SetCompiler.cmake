...
IF (CMAKE_Fortran_COMPILER_ID MATCHES "GNU")
  ...
  IF (NOT DEFINED C_FLAGS_INITIALIZED )
    ...
    IF(WIN32)
      # On Windows/MinGW-UCRT64: omit -finit-real=snan and -finit-integer=snan as they
      # trigger internal compiler errors (segfault in f951.exe) with gfortran 15.x on Windows.
      # libstdc++ is linked automatically by MinGW; -lstdc++ is not needed.
  ...
  # Note: -fstack-arrays causes internal compiler errors (ICE) in gfortran on Windows/MinGW with LTO
  IF(WIN32)
    ...
    SET (CMAKE_Fortran_FLAGS_NOGPROFILE  "${CMAKE_Fortran_FLAGS} -g1 -O3 ${PICLAS_INSTRUCTION} -fno-inline -fno-optimize-sibling-calls")
    SET (CMAKE_Fortran_FLAGS_DEBUG       "${CMAKE_Fortran_FLAGS} -g  -Og -ggdb3 -ffpe-trap=invalid,zero,overflow -fbounds-check -finit-real=snan -fbacktrace -Wall")
    SET (CMAKE_Fortran_FLAGS_SANITIZE    "${CMAKE_Fortran_FLAGS} -g  -Og -ggdb3 -ffpe-trap=invalid,zero,overflow,denorm -fbounds-check -fbacktrace -Wall -fno-omit-frame-pointer -Wc-binding-type -Wuninitialized -pedantic")
  ELSE()
    SET (CMAKE_Fortran_FLAGS_NITRO       "${CMAKE_Fortran_FLAGS}  -Ofast ${PICLAS_INSTRUCTION}")
...
# Intel oneAPI (ifx) Compiler - classic ifort uses "Intel", ifx uses "IntelLLVM"
ELSEIF (CMAKE_Fortran_COMPILER_ID MATCHES "IntelLLVM")
  IF (NOT DEFINED C_FLAGS_INITIALIZED )
    SET (C_FLAGS_INITIALIZED "yes" CACHE INTERNAL "Flag if compiler flags are already initialized" )
    IF(WIN32)
      # On Windows: no -traceback, use /traceback; no -warn, use /warn; ifx Windows flags
      SET (CMAKE_Fortran_FLAGS       "${CMAKE_Fortran_FLAGS} -real-size:64 -integer-size:32 /traceback /warn:all /DINTEL")
    ELSE()
      SET (CMAKE_Fortran_FLAGS       "${CMAKE_Fortran_FLAGS} -real-size 64 -integer-size 32 -traceback -warn all -lstdc++ -DINTEL")
    ENDIF()
  ENDIF()
  SET (CMAKE_Fortran_FLAGS_NITRO     "${CMAKE_Fortran_FLAGS}      -O3 ${PICLAS_INSTRUCTION} -no-prec-div")
  SET (CMAKE_Fortran_FLAGS_RELEASE   "${CMAKE_Fortran_FLAGS}      -O3 ${PICLAS_INSTRUCTION} -no-prec-div")
  SET (CMAKE_Fortran_FLAGS_PROFILE   "${CMAKE_Fortran_FLAGS}  -p  -O3 ${PICLAS_INSTRUCTION} -no-prec-div")
  SET (CMAKE_Fortran_FLAGS_DEBUG     "${CMAKE_Fortran_FLAGS}  -g  -O0 -fpe0 -traceback -check all,noarg_temp_created,noformat,nooutput_conversion,pointer,uninit -init=snan -init=arrays")
  IF(NOT "${CMAKE_GENERATOR}" MATCHES "Ninja")
    IF(WIN32)
      SET (PICLAS_COMPILE_FLAGS "/fpp")
    ELSE()
      SET (PICLAS_COMPILE_FLAGS "-fpp -allow nofpp_comments -assume bscc")
    ENDIF()
  ELSE()
...
ELSE()
  MESSAGE(SEND_ERROR "Unknown compiler '${CMAKE_Fortran_COMPILER_ID}'. Supported compilers: GNU, Flang, Intel, IntelLLVM, Cray.")
ENDIF()
